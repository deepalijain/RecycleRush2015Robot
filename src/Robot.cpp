...
IMAQdxSession Robot::session;
Image *Robot::frame;
IMAQdxError Robot::imaqError;
...
void Robot::CameraStart() {
	std::string imaqErrorString;
	IMAQdxCameraInformation camInfo[6]; 	// we crash if there are more than 6 cameras!
	uInt32 cameraCount=6;
	...
	if(imaqError != IMAQdxErrorSuccess) {
		...
		printf("IMAQdxOpenCamera error: %s\n", imaqErrorString.c_str());
	}
	imaqError = IMAQdxConfigureGrab(session);
	if(imaqError != IMAQdxErrorSuccess) {
		imaqErrorString = std::to_string((long)imaqError);
		DriverStation::ReportError("IMAQdxConfigureGrab error: " + imaqErrorString + "\n");
		printf("IMAQdxConfigureGrab error: %s\n", imaqErrorString.c_str());
	}
    // start to acquire images
	printf("Camera Session: %lu", session);
	if (0!=session) IMAQdxStartAcquisition(session);


}

void Robot::CameraFeed() {
	std::string imaqErrorString;
